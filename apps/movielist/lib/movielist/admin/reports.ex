defmodule Movielist.Reports do
  ...
  def movie_stats_for_genre(genre_id) do
    from(m in Movie,
      ...
      select: %{movie_count: count(m), average_pre_rating: avg(m.pre_rating)}
    )
    |> Repo.one!()
  end
  ...
  def rating_stats_for_genre(genre_id) do
    from(r in Rating,
      ...
      where: m.genre_id == ^genre_id,
      select: %{rating_count: count(r), average_score: avg(r.score)}
    )
    |> Repo.one!()
  end
  ...
  """
  def calculate_ratings_per_month(ratings, year, current_date) do
    end_month =
      case year == current_date.year do
        true -> current_date.month
        false -> 12
      end
    ...
    |> Enum.map(fn month_number ->
      {month_number, Map.get(month_map, month_number, 0)}
    end)
  end
end
