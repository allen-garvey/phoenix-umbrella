defmodule Artour.Guggenheim.Image do
    ...
    def create_liquid_thumbnails(temp_dir, source_image_models) do
        ...
        |> Enum.map(fn image_path -> 
            {_res, 0} = System.cmd("convert", [image_path, "-liquid-rescale", "#{@thumbnail_size}x#{@thumbnail_size}!", "-quality", "40%", "-set", "filename:name", "%t", Path.join(temp_dir, "%[filename:name]-a.jpg")])
