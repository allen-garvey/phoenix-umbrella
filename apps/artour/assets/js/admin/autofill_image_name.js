...
export function initializeAutofillImageName() {
    ...
    const TEXTFIELD_TARGETS = ['large', 'medium', 'small', 'thumbnail'].map(
        function (elem) {
    ...
    function autofillFields(inputText, shouldForce) {
        ...
        const fileSuffixes = isPortraitCheckbox.checked
            ? FILE_SUFFIXES_PORTRAIT
            : FILE_SUFFIXES;

        TEXTFIELD_TARGETS.forEach(function (textfield, i) {
            autofillTextfield(
                textfield,
                suffixToImageFilename(imageName, fileSuffixes[i]),
                shouldForce
            );
        });
    }
