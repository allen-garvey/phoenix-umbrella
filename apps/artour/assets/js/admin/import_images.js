...
export function extractImages(files) {
    ...
    for (let file of files) {
        if (file.type.match(/^image\//)) {
            ...
            const title = titleizeStringArray(
                nameDashSplit.slice(0, nameDashSplit.length - 1)
            );
            const sizeSuffix = nameDashSplit[nameDashSplit.length - 1];
            const uniqueName = nameDashSplit
                .slice(0, nameDashSplit.length - 1)
                .join('-');

            const fileObject = imageFilesMap.has(uniqueName)
                ? imageFilesMap.get(uniqueName)
                : {};
            fileObject[sizeSuffix] = name;
            fileObject.slug = uniqueName;
            fileObject.title = title;
            if (!fileObject.thumb) {
                fileObject.thumb = `${uniqueName}-thumb.webp`;
            }

            if (
                sizeSuffix === 'thumb' ||
                (!fileObject.file && sizeSuffix === 'sm') ||
                !fileObject.file
            ) {
